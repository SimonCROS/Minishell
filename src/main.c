...
void	term_load(void)
{
	...
	if (tcgetattr(0, &global.shell.term) == ERROR || tcgetattr(0, &global.shell.save) == ERROR)
	{
		ft_putendl_fd(strerror(errno), 2);
		exit(1);
	}
	global.shell.term.c_lflag &= ~(ICANON | ECHO);
	global.shell.term.c_cc[VMIN] = 1;
    global.shell.term.c_cc[VTIME] = 0;
	if (tcsetattr(0, TCSANOW, &global.shell.term) == ERROR)
	{
		ft_putendl_fd(strerror(errno), 2);
		exit(1);
	}
